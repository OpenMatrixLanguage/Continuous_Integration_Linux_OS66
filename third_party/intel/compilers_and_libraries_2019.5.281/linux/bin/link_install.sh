...
# link_install.sh - makes symbolic links and other environment changes 
...
ICP12_CREATE_DIR_LINK() {
    ...
    if [ -L $THIS_NEW_LINK_DIR ]; then
        ...
        # if we have old libraries folders installed (such directories exist in /opt/intel), then we
...
ICP12_MKLINKS() {
    ...
    ICP12_COPY_FILE $FULL_PROGRAM_PATH $INTEL_COMPLIB_DIR/linux/bin

    # check which version directories "lib", "include" points to and try to set up all links in bin by binaries from this version
    CPRO_VER_SET=$(readlink $INTEL_COMPLIB_2019_LIB_DIR | sed "s/^.*\(compilers_and_libraries_2019.[0-9]\.[0-9][0-9][0-9]\).*$/\1/g")
    ...
    [ -d $PKG_INSTALL_DESTINATION/linux/bin/ia32 ] && mkdir -p $INTEL_COMPLIB_2019_BIN_DIR/ia32
    #create folder for intel64 links
    [ -d $PKG_INSTALL_DESTINATION/linux/bin/intel64 ] && mkdir -p $INTEL_COMPLIB_2019_BIN_DIR/intel64
	            	
    # create links for binary and non-env script program files
    for THIS_FILE in $INTEL_PROGRAM_FILES_LIST
    do
        
        # creating links on files in INTEL_COMPLIB_2019_BIN_DIR dir for ia32 architecture
        pushd $INTEL_COMPLIB_2019_BIN_DIR/ia32 &> /dev/null
        ICP12_CREATE_FILE_LINK ../../../../$PACKAGEID/linux/bin/ia32/$THIS_FILE $INTEL_COMPLIB_2019_BIN_DIR/ia32/$THIS_FILE
        popd &>/dev/null
        pushd $INTEL_COMPLIB_2019_BIN_DIR/intel64 &> /dev/null
        ICP12_CREATE_FILE_LINK ../../../../$PACKAGEID/linux/bin/intel64/$THIS_FILE $INTEL_COMPLIB_2019_BIN_DIR/intel64/$THIS_FILE
        popd &>/dev/null
        
		# creating links on files in INTEL_BIN_DIR dir
        pushd $INTEL_BIN_DIR &>/dev/null
        ICP12_CREATE_FILE_LINK ../compilers_and_libraries_2019/linux/bin/$ARCH/$THIS_FILE $INTEL_BIN_DIR/$THIS_FILE
        popd &> /dev/null
    done
    ...
    do
        ...
        if [ "x$(echo $THIS_FILE | grep compilervars)" != "x" ]; then
            ...
            cp -p ${filetarget} "$INTEL_COMPLIB_2019_BIN_DIR/${filedestination}" &>/dev/null
            VERBOSE_PRINT "Copied $THIS_FILE to $INTEL_COMPLIB_2019_BIN_DIR/${filedestination}"

            pushd $INTEL_BIN_DIR &>/dev/null
            ICP12_CREATE_FILE_LINK ../compilers_and_libraries_2019/linux/bin/${filedestination} $INTEL_BIN_DIR/${filedestination}
            popd &> /dev/null
        else
    ...
    pushd $INTEL_COMPLIB_2019_DIR &>/dev/null
...
ICP12_RMLINKS() {
    ...
    # support of SC foldermap
    if [ -e "${INTEL_COMPLIB_2019_DIR}/foldermap.sc.xml" ] ; then
        rm $RM_OPTIONS "${INTEL_COMPLIB_2019_DIR}/foldermap.sc.xml" &>/dev/null
    fi
    # support of SC foldermap - end
...
FIND_LATEST_PROD_INSTANCE () {
    ...
    CPRO_INSTALLED_VERSIONS=$(ls -A $TOP_INTEL_DIR | grep 'compilers_and_libraries_' | sed 's/compilers_and_libraries_//g' | grep -e '2019' | grep -ve '^2019$' | sort -gr)
    ...
    if [ -z "$CPRO_INSTALLED_VERSIONS" ]; then
        ...
        LATEST_VER=$(echo "$CPRO_INSTALLED_VERSIONS" | grep -e "^2019\.[0-9]*\.${CPRO_LATEST_PACKAGE_NUMBER}$")
    fi
...
d n  INTEL_COMPLIB_2019_DIR                     \$TOP_INTEL_DIR/compilers_and_libraries_2019;\
d n  INTEL_COMPLIB_2019_Lin_DIR                 \$TOP_INTEL_DIR/compilers_and_libraries_2019/linux;\
d n  INTEL_COMPLIB_2019_BIN_DIR                 \$INTEL_COMPLIB_2019_Lin_DIR/bin;\
l n  INTEL_COMPLIB_DIR                          \$TOP_INTEL_DIR/compilers_and_libraries         compilers_and_libraries_2019;\
l n  INTEL_COMPLIB_2019_PKG_BIN_DIR             \$INTEL_COMPLIB_2019_Lin_DIR/pkg_bin            ../../$PACKAGEID/linux/bin;\
l n  INTEL_COMPLIB_2019_LIB_DIR                 \$INTEL_COMPLIB_2019_Lin_DIR/lib                ../../$PACKAGEID/linux/compiler/lib;\
l n  INTEL_COMPLIB_2019_INCLUDE_DIR             \$INTEL_COMPLIB_2019_Lin_DIR/include            ../../$PACKAGEID/linux/compiler/include;\
l y  INTEL_COMPLIB_2019_TBB_DIR                 \$INTEL_COMPLIB_2019_Lin_DIR/tbb                ../../$PACKAGEID/linux/tbb;\
l y  INTEL_COMPLIB_2019_PSTL_DIR                \$INTEL_COMPLIB_2019_Lin_DIR/pstl               ../../$PACKAGEID/linux/pstl;\
l y  INTEL_COMPLIB_2019_MKL_DIR                 \$INTEL_COMPLIB_2019_Lin_DIR/mkl                ../../$PACKAGEID/linux/mkl;\
l y  INTEL_COMPLIB_2019_IPP_DIR                 \$INTEL_COMPLIB_2019_Lin_DIR/ipp                ../../$PACKAGEID/linux/ipp;\
l y  INTEL_COMPLIB_2019_IPPCP_DIR               \$INTEL_COMPLIB_2019_Lin_DIR/ippcp              ../../$PACKAGEID/linux/ippcp;\
l y  INTEL_COMPLIB_2019_MPIRT_DIR               \$INTEL_COMPLIB_2019_Lin_DIR/mpirt              ../../$PACKAGEID/linux/mpirt;\
l y  INTEL_COMPLIB_2019_MPI_DIR                 \$INTEL_COMPLIB_2019_Lin_DIR/mpi                ../../$PACKAGEID/linux/mpi;\
l y  INTEL_COMPLIB_2019_DAAL_DIR                \$INTEL_COMPLIB_2019_Lin_DIR/daal               ../../$PACKAGEID/linux/daal;\
l y  INTEL_COMPLIB_2019_DEBUGGER_DIR            \$INTEL_COMPLIB_2019_Lin_DIR/debugger           ../../debugger_2019;\
l n  INTEL_COMPLIB_2019_DOCUMENTATION_DIR       \$INTEL_COMPLIB_2019_Lin_DIR/documentation      ../../documentation_2019;\
l n  INTEL_COMPLIB_2019_MAN_DIR                 \$INTEL_COMPLIB_2019_Lin_DIR/man                documentation/en/man;\
...
ICP12_USAGE() {
    ...
    echo "  -p PATH_TO_PACKAGE  - full path to the installed package"
    echo "         (e.g., root)"
    ...
    echo
    echo "Copyright 2006-2019, Intel Corporation, All Rights Reserved."
}
...
#DEFAULT_PACKAGEID="compilers_and_libraries_2019.0.001"
DEFAULT_PACKAGEID="compilers_and_libraries_2019.5.281"

PACKAGEID=$DEFAULT_PACKAGEID

PACKAGENUM=$(echo $PACKAGEID | sed -e"s/compilers_and_libraries_2019\.[0-9]\.//g")
...
if [ -z "$TOP_INTEL_DIR" ]; then
    ...
    if [ $(id -u) -eq 0 ]; then
        ...
        #exit 1
        #Behavior was changed according to DPD200254198
